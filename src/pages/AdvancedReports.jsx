...
export default function AdvancedReports() {
  ...
  const collectionData = useMemo(() => {
    ...
    filtered.forEach(p => {
      ...
      if (p.payment_status === 'completed') completedAmount += a;
      if (p.payment_status === 'pending')   pendingAmount   += a;
    });
  ...
  const cancellationData = useMemo(() => {
    ...
    const cancelled = filtered.filter(a => a.status === 'cancelled' || a.status === 'missed');
    ...
    filtered.forEach(a => {
      ...
      if (byDay[day]) {
        byDay[day]['סה״כ']++;
        if (a.status === 'cancelled' || a.status === 'missed') byDay[day]['ביטולים']++;
      }
    });
