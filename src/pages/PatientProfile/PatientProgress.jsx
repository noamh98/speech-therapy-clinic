...
import { formatDate, PROGRESS_TYPES, PROGRESS_DOMAINS } from '../../utils/formatters';
...
export default function PatientProgress({ patient }) {
  ...
  const [form, setForm] = useState({ date: new Date().toISOString().slice(0, 10), type: 'goal', title: '', domain: 'speech', score: 5, description: '', notes: '' });
  ...
  async function load() {
    setLoading(true);
    try {
      const q = query(collection(db, 'progress'), where('patient_id', '==', patient.id), orderBy('date', 'asc'));
        ...
        <div className="p-4 bg-gray-50 rounded-xl">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={chartData}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="score" stroke="#14b8a6" strokeWidth={2} dot={{ r: 4, fill: '#14b8a6' }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {records.length === 0 ? (
        ...
        <div className="space-y-2">
          {[...records].reverse().map(r => {
            ...
            return (
              <motion.div key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-3 border border-gray-100 rounded-xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select className="input" value={form.type} onChange={set('type')}>
                {PROGRESS_TYPES.map(t => <option key={t.value} value={t.value}>{t.icon} {t.label}</option>)}
              </select>
            </div>
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="input" value={form.domain} onChange={set('domain')}>
                {PROGRESS_DOMAINS.map(d => <option key={d.value} value={d.value}>{d.label}</option>)}
              </select>
            </div>
            <div>
              <label className="label">ציון (1–10)</label>
              <input type="number" className="input" min={1} max={10} value={form.score} onChange={set('score')} />
            </div>
          </div>
          <div>
