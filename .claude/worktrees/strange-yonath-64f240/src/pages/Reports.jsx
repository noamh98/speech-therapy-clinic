...
export default function Reports() {
  ...
  const stats = useMemo(() => {
    const completedPayments = filteredPayments.filter(p => p.payment_status === PAYMENT_STATUS.COMPLETED);
    const totalIncome = completedPayments.reduce((s, p) => s + (Number(p.amount) || 0), 0);
    const pendingIncome = filteredPayments.filter(p => p.payment_status === PAYMENT_STATUS.PENDING).reduce((s, p) => s + (Number(p.amount) || 0), 0);
    ...
    const cancelRate = filteredAppointments.length 
      ? ((filteredAppointments.filter(a => a.status === PAYMENT_STATUS.CANCELLED || a.status === 'missed').length / filteredAppointments.length) * 100).toFixed(1) 
      : 0;
  ...
  const pieData = useMemo(() => {
    const map = {};
    filteredPayments.forEach(p => {
      if (p.payment_status !== PAYMENT_STATUS.COMPLETED) return;
